package com.raptor.sdu.render.tileentity;
...
public class TileEntityDrawersUnlimitedRenderer extends TileEntitySpecialRenderer<TileEntityDrawers> {
	...
	private void renderFastItemSet(ChamRender renderer, TileEntityDrawers tile, IBlockState state, EnumFacing side,
		...
		if(tile.getDrawerAttributes().isShowingQuantity()) {
			...
			if(distance < 10) {
				for(int i = 0; i < drawerCount; i++) {
					String text = CountFormatter.format(getFontRenderer(), tile.getDrawer(i));
					renderText(text, tile, state, i, side, depth, alpha);
				}
			}
		}
	}

	private void renderText(String text, TileEntityDrawers tile, IBlockState state, int slot, EnumFacing side,
			float depth, float alpha) {
		if(text == null || text.isEmpty())
			return;

		BlockDrawers block = (com.raptor.sdu.block.BlockUnlimitedDrawers)state.getBlock();
		...
		//getFontRenderer().drawString("test", 0, 0, 0xFF_FFFFFF);
	...
	private void renderFastItem(ChamRender renderer, @Nonnull ItemStack itemStack, TileEntityDrawers tile,
			IBlockState state, int slot, EnumFacing side, float depth, float partialTickTime) {
		int drawerCount = tile.getDrawerCount();
		...
		// At the time GL_LIGHT* are configured, the coordinates are transformed by the
		// modelview
		// matrix. The transformations used in
		// `RenderHelper.enableGUIStandardItemLighting` are
		// suitable for the orthographic projection used by GUI windows, but they are
		// just a little
		// bit off when rendering a block in 3D and squishing it flat. An additional
		// term is added
		// to account for slightly different shading on the half-size "icons" in 1x2 and
		// 2x2
		// drawers due to the extreme angles caused by flattening the block (as noted
		// below).
		...
		// TileEntitySkullRenderer alters both of these options on, but does not restore
		// them.
		GlStateManager.enableCull();
		GlStateManager.disableRescaleNormal();

		// GL_POLYGON_OFFSET is used to offset flat icons toward the viewer (-Z) in
		// screen space,
		// so they always appear on top of the drawer's front space.
		GlStateManager.enablePolygonOffset();
		GlStateManager.doPolygonOffset(-1, -1);

		// DIRTY HACK: Fool GlStateManager into thinking GL_RESCALE_NORMAL is enabled,
		// but disable
		// it using popAttrib This prevents RenderItem from enabling it again.
		//
		// Normals are transformed by the inverse of the modelview and projection
		// matrices that
		// excludes the translate terms. When put through the extreme Z scale used to
		// flatten the
		// block, this makes them point away from the drawer face at a very sharp angle.
		// These
		// normals are no longer unit scale (normalized), and normalizing them via
		// GL_RESCALE_NORMAL causes a loss of precision that results in the normals
		// pointing
		// directly away from the face, which is visible as the block faces having
		// identical
		// (dark) shading.
	...
	private void renderIndicator(ChamRender renderer, TileEntityDrawers tile, IBlockState blockState, int side,
			EnumUpgradeStatus level) {
		...
		for(int i = 0; i < count; i++) {
			...
			TextureAtlasSprite iconOff = Chameleon.instance.iconRegistry
					.getIcon(statusInfo.getSlot(i).getOffResource(level));
			TextureAtlasSprite iconOn = Chameleon.instance.iconRegistry
					.getIcon(statusInfo.getSlot(i).getOnResource(level));
			...
			renderer.setRenderBounds(statusArea.getX() * unit, statusArea.getY() * unit, 0,
					(statusArea.getX() + statusArea.getWidth()) * unit,
					(statusArea.getY() + statusArea.getHeight()) * unit, depth - frontDepth);
			...
			if(level == EnumUpgradeStatus.LEVEL1 && !drawer.isEmpty() && drawer.getRemainingCapacity() == 0) {
				...
				double indXCur = (stepX == 0)? indXEnd
						: getIndEnd(block, tile, i, indXStart, activeArea.getWidth(), stepX);

				double indYStart = activeArea.getY();
				double indYEnd = activeArea.getY() + activeArea.getHeight();
				double indYCur = (stepY == 0)? indYEnd
						: getIndEnd(block, tile, i, indYStart, activeArea.getHeight(), stepY);
	...
	private void renderTape(ChamRender renderer, TileEntityDrawers tile, IBlockState blockState, int side,
			boolean taped) {
		...
		renderer.state.setRotateTransform(ChamRender.ZPOS, side);
		renderer.renderPartialFace(ChamRender.FACE_ZPOS, null, blockState, BlockPos.ORIGIN, iconTape, 0, 0, 1, 1, 1, 1,
				1);
		renderer.state.clearRotateTransform();
